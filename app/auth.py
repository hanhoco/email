...
@bp.route('/register', methods=('GET','POST')) #?(3)
def register():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))
        ...
        if request.method == 'POST': #?(4)   
            username = request.form['username'] #?(5)
            password = request.form['password'] #?(6)
            email = request.form['email'] #?(7)
            ...
            if (not email or (not utils.isEmailValid(email))): #?(11)
...
@bp.route('/confirm', methods=('GET', 'POST')) #?(12)
def confirm():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))
        ...
        if request.method == 'POST':  #?(13)
            password = request.form['password']  #?(14)
            password1 = request.form['password1'] #?(15)
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            email = request.form['email'] #?(21)
            
            if ((not email) or (not utils.isEmailValid(email))): #?(22)
                error = 'Email Address Invalid'
                flash(error)
                return render_template('auth/forgot.html')
...
@bp.route('/login', methods=('GET','POST')) #?(23)
def login():
    try:
        if g.user:
            return redirect(url_for('inbox.show'))

        if request.method == 'POST': #?(24)
            username = request.form['username'] #?(25)
            password = request.form['password'] #?(26)
...
def load_logged_in_user():
    user_id = session.get('user_id') #?(32)
